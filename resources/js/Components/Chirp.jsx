            ...
            <div className="flex-1">
                <div className="flex justify-between items-center">
                    <div>
                        ...
                        {/* Compare Chirp Created At adn Chirp Updated At, if not same then display edited component*/}
                        { chirp.created_at !== chirp.updated_at && <small className="text-sm text-gray-600"> &middot; edited</small>}
                    </div>

                    {/* Only display dropdown button just only to the chirper user */}
                    {chirp.user.id === auth.user.id &&
                        <Dropdown>
                            ...
                            <Dropdown.Content>
                                {/* If Click this button, we set setEditing value to true */}
                                <button className="block w-full px-4 py-2 text-left text-sm leading-5 text-gray-700 hover:bg-gray-100 focus:bg-gray-100 transition duration-150 ease-in-out" onClick={() => setEditing(true)}>
                                    Edit
                                </button>
                                {/* If clic this link, we redirect to destroy route and Delete this chirp */}
                ...
                {/* Show Editing component if on editing phase, if not then show chirp message */}
                {editing
